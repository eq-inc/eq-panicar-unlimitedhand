...
import jp.eq_inc.aranduh.fragment.AROnCameraWithGLFragment;

public class AROnCameraWithGLActivity extends AppCompatActivity {
    @Override
            ...
            if (enableLaunch) {
                ...
                if (manager.findFragmentByTag(null) == null) {
                    manager.beginTransaction()
                            .add(R.id.container, new AROnCameraWithGLFragment())
                            .commit();
                }
            } else {
                ToastUtil.showToast(AROnCameraWithGLActivity.this, "cannot launch by not allowed permissions", Toast.LENGTH_LONG);
                finish();
            }
        }
    };
